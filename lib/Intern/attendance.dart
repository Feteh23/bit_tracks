...
Future<void> _loadAttendanceRecords() async {
  ...
  setState(() {
    ...
    for (var doc in snapshot.docs) {
      ...
      final normalizedDate = DateTime(timestamp.toDate().year, timestamp.toDate().month, timestamp.toDate().day);
      attendanceRecords[normalizedDate] = AttendanceEntry(
...
Future<void> _saveAttendanceDescription(DateTime date, String description, String status) async {
  ...
  if (existingDoc.exists) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text("Log book has already been filled for this day.", style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  await docRef.set({
    // Store date as a Timestamp in Firestore (Flutter's DateTime converts automatically)
    'date': date,
    'description': description,
    'isPresent': true, // Assuming filling logbook means present
    'timestamp': FieldValue.serverTimestamp(),
  });

  // Update the local attendanceRecords map and then the pie chart
  final DateTime normalizedDate = DateTime(date.year, date.month, date.day);
  setState(() {
    attendanceRecords[normalizedDate] = AttendanceEntry(isPresent: true, description: description);
    _updatePieChartData(); // Recalculate pie chart percentages
  });
...
void _openAttendanceModal(BuildContext context, DateTime selectedDay) {
  ...
  if (attendanceRecords.containsKey(normalizedSelectedDay)) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text("Log book has already been filled for this day.", style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.orange,
  ...
  AttendanceStatus attendanceStatus = AttendanceStatus.present; // ✅ Declare here
        ...
        builder: (BuildContext context, StateSetter setState) {
          return Padding(
            ...
            child: Column(
              ...
              children: [
                Text("Mark Attendance", style: TextStyle(fontSize: screenWidth * 0.05, fontWeight: FontWeight.bold)),
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () async {
                    try {
                      await _saveAttendanceDescription(
                        selectedDay,
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: const Text("Log book update saved successfully!", style: TextStyle(fontWeight: FontWeight.bold)),
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text("Failed to save the log book update: $e", style: const TextStyle(fontWeight: FontWeight.bold)),
...
body: Column(
  ...
  if (selectedDate == currentDate) {
    final alreadyMarked = attendanceRecords.containsKey(selectedDate);

    if (alreadyMarked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Attendance already marked for today.", style: TextStyle(fontSize: screenWidth * 0.04, fontWeight: FontWeight.bold)),
          backgroundColor: Colors.orange,
        ),
      );
    } else {
